...
class NeoModelConversions:
    ...
    def create_or_update_locations(*locations: LocationEntity):
        ...
        QUERY = """
            ...
            DELETE oldRel
            MERGE (lm:LandmarkNode {uid: row.landmark_id})
            WITH row, loc, lm
    ...
    def create_or_update_items(*items: ItemEntity):
        ...
        QUERY = """
            ...
            //UPDATE HOLDER REL
            WITH row, i
            OPTIONAL MATCH (i)-[oldRel:HOLDER]->(:HolderNode)
            DELETE oldRel
            WITH row, i
    ...
    @classmethod
    def hydrate_entity(cls, labels: List[str], props: Dict[str,str]) -> Entity:
        for label in labels:
    ...
    def load_episode_from_landmark(cls, landmark_id: str) -> Episode:
        ...
        intermediate = [cls.hydrate_entity(list(n.labels), n._properties) for n in records[0][0]]
        ...
        return Episode(
            landmark=LandmarkEntity.model_validate(landmark),
            locations=[LocationEntity.model_validate(location) for location in locations.values()],
            junctions=[JunctionEntity.model_validate(junction) for junction in junctions],
            actors=[ActorEntity.model_validate(actor) for actor in actors.values()],
            items=[ItemEntity.model_validate(item) for item in items.values()],
            actions = [],
            outcomes = [],
        )
