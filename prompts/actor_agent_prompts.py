...
def build_actor_agent_prompt(
    ...
    if(location):
        ...
        prompt += "\nACTORS\n"
        prompt += f"{[a.get_observable_details_str() for a in actors]}\n"
        prompt += "\nHELD ITEMS\n"
        prompt += f"{ {a: [i.get_observable_details_str() for i in il] for a,il in held_item_map.items()} }\n"
        prompt += "\nDROPPED ITEMS\n"
        prompt += f"{[i.get_observable_details_str() for i in dropped_items]}\n"
    ...
    if(current_episode):
        ...
        prompt += f"{current_episode.event_history[len(actors)*-2:]}\n"
